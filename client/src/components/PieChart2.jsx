...
const PieChart2 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(
                    `http://127.0.0.1:8000/charts/delay-pie-sub?airport=${selectedAirport}&start_date=${startDate}&end_date=${endDate}`
                );
        ...
        fetchData();
    }, [startDate, endDate, selectedAirport]); // Run only once on mount
    ...
    return (
        <div style={{ width: '50%', height: '43.5vh' }}>
            <Plot
                data={[
                    {
                        ...
                        marker: {
                            colors: [
                                '#9B59B6', // Light Purple 8E44AD --> air system
                                'Black', // Purple --> security
                                '#CC86E2', // Dark Purple --> airline
                                '#9D16C8', // Very Dark Purple --> weather
                                '#6A1F9C', // Deep Purple --> late aircraft 
